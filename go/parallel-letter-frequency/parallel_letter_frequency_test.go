...
func BenchmarkSequentialFrequency(b *testing.B) {
	for i := 0; i < b.N; i++ {
		Frequency(euro + dutch + us + euro + dutch + us + euro + dutch + us + euro + dutch + us + euro + dutch + us)
	}
}

func BenchmarkConcurrentFrequency(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ConcurrentFrequency([]string{euro, dutch, us, euro, dutch, us, euro, dutch, us, euro, dutch, us, euro, dutch, us})
	}
}

func BenchmarkConcurrentFrequency2(b *testing.B) {
	for i := 0; i < b.N; i++ {
		ConcurrentFrequency2([]string{euro, dutch, us, euro, dutch, us, euro, dutch, us, euro, dutch, us, euro, dutch, us})
	}
}
